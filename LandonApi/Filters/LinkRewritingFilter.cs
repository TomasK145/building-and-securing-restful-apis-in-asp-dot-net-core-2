...
{
    //result filter --> pre non-exceptional responses, bezi pred a po tom ako je result spracovany, umoznuje intercept response predtym ako je odoslana clientovi
    public class LinkRewritingFilter : IAsyncResultFilter
    {
        private readonly IUrlHelperFactory _urlHelperFactory; //umoznuje vytvorit IUrlHelper on-the-fly --> urlHelperFactory.GetUrlHelper(context)
        ...
        {
            var asObjectResult = context.Result as ObjectResult; //result z controlleru
            bool shouldSkip = asObjectResult?.StatusCode >= 400
                                || asObjectResult?.Value == null
                                || !(asObjectResult.Value is Resource); //kontrola ci sa jenda o resource type

            if (shouldSkip)
            {
                return next(); //vratenenie next() zabezpeci moznost pokracovania v spracovani response
            }
